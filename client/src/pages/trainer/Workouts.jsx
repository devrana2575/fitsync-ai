          ...
          <div className="bg-white rounded-xl border border-slate-200 p-6 mb-8">
            ...
            <form onSubmit={handleSubmit} className="space-y-4">
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <div>
                  <label className="block text-sm font-medium text-slate-700 mb-1">Recommendation Source</label>
                  <select
              ...
              <table className="w-full">
                <thead className="bg-slate-50">
                  <tr>
                    ...
                    <th className="px-6 py-3 text-left text-xs font-medium text-slate-500 uppercase tracking-wider">Hours / Day</th>
                ...
                <tbody className="divide-y divide-slate-200">
                  {plans.map((plan) => (
                    <tr key={plan._id || plan.id} className="hover:bg-slate-50 transition-colors">
                      ...
                      <td className="px-6 py-4 text-sm text-slate-500">{plan.hoursPerDay != null ? plan.hoursPerDay : '—'}</td>
                      <td className="px-6 py-4 text-sm text-slate-500">{plan.totalHours != null ? plan.totalHours : '—'}</td>
                      <td className="px-6 py-4 text-sm text-slate-500">{plan.exercises?.length || 0}</td>
                      <td className="px-6 py-4 text-sm text-slate-500">{plan.recommendationSource ? plan.recommendationSource.charAt(0).toUpperCase() + plan.recommendationSource.slice(1) : '—'}</td>
